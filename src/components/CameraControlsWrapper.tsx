...
export default function CameraControlsWrapper() {
  ...
  useEffect(() => {
    ...
    controls.minPolarAngle = 0; // Minimum vertical rotation angle (to prevent going under ground) - 垂直回転の最小角度（地面以下に潜らないように）
  ...
  useFrame((_, delta) => {
    if (controlsRef.current) {
      controlsRef.current.update(delta); // Update position and rotation (位置と回転を更新)
