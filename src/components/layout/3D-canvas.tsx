...
const WebGLFallback = () => (
  <div className="w-full h-full flex items-center justify-center bg-gray-900">
    <div className="text-white text-center p-8 max-w-md">
      <h2 className="text-2xl font-bold mb-4">WebGL がサポートされていません</h2>
      <p className="mb-4">
  ...
  const CanvasErrorFallback = () => (
    <div className="w-full h-full flex items-center justify-center bg-gray-900">
      <div className="text-white text-center p-8 max-w-md">
        ...
        <p className="mb-4">
          {error?.message || "3Dエクスペリエンスの読み込み中にエラーが発生しました。"}
        </p>
        <p>
          ページを再読み込みするか、別のブラウザをお試しください。
        </p>
      </div>
    </div>
  );
    ...
    <ErrorBoundary>
      ...
      >
        <Canvas
          ...
          camera={{ fov: 25, far: 2000, near: 0.1, position: [10, 5, 10] }} // カメラ設定の最適化 - Optimized camera settings
          gl={{ 
        ...
        >
          ...
          {/* OrbitControls: カメラ操作のためのコントロール - OrbitControls for camera interaction */}
          <OrbitControls
            makeDefault // デフォルトのコントローラーを設定 - Ensures this is the default controller
            autoRotate // シーンを優しく回転 - Gently rotate the scene
            autoRotateSpeed={0.5} // 自動回転速度 - Auto-rotation speed
            enableZoom={true} // ズーム有効化 - Enable zoom
            enablePan={true} // パン有効化 - Enable pan
            minPolarAngle={Math.PI / 4} // 垂直回転制限 - Limit vertical rotation
            maxPolarAngle={Math.PI / 2} // 垂直回転制限 - Limit vertical rotation
          />
